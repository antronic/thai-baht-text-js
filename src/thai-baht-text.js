...
const convertFullMoney = numberInput => {
  ...
  const textOutput = [intTextOutput && `${intTextOutput}${PRIMARY_UNIT}`, fractionalPart && convert(reverseNumber(fractionalPart)) && `${convert(reverseNumber(fractionalPart))}${SECONDARY_UNIT}`].filter(Boolean).join('');
